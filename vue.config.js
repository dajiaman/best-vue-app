...
// 获取所有模块
const getModulesCount = () => fs.readdirSync("./modules/").length;
// 获取编译模块
...
if (modules && process.env.NODE_ENV === "development") {
    ...
    buildDynamicModules = modules.split(",").map((module, index) => {
        ...
        fs.access(path.join(`./modules/${module}/index.js`), function(err) {
            if (err) {
                console.log(`\n\n注意：${module} 模块不存在!\n`);
                process.exit(0);
            }
    ...
    buildDynamicModules.push(`require.context("../modules/", true, /index\.js$/)`);
}
// 创建虚拟模块文件（hack）
    ...
    css: {
        ...
        loaderOptions: {
            less: {
                ...
                // javascriptEnabled: true,
                additionalData: `@env: ${process.env.VUE_APP_BASE_API};`,
            },
